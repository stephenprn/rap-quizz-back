...
from app.repositories import ResponseRepository, FilterLabel
...
def get_list_from_search_txt(search_txt: str, type_: ResponseType, responses_uuid_exclude: List[str]) -> Response:
    search_txt = normalize_string(search_txt)
    return repo_response.list_(
        filter_search_text=search_txt, filter_type_in=[
            type_], filter_uuid_not_in=responses_uuid_exclude, nbr_results=RESPONSES_LIST_SEARCH_TXT_NBR_RESULTS
    )


def add_simple(label: str, type_: ResponseType) -> Response:
    if repo_response.get(filter_label=FilterLabel(label=label, ignore_case=True), filter_type_in=[type_]) != None:
...
def add(response: Response):
    if repo_response.get(filter_label=FilterLabel(label=response.label, ignore_case=True), filter_type_in=[response.type]) != None:
